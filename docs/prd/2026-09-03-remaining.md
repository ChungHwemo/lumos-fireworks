...
- **지도 조작.** MapLibre 기본 컨트롤(29px, 영어, 회전은 컴패스 드래그에만 숨어 있음)을 걷어내고 `MapControls` 로 바꿨다. 시트 위 오른쪽에 세로로 선다: 컴패스(방위 표시, 탭=북쪽, 끌기=회전·기울기), ⟲⟳ 45°, 2D/3D, 자동 회전 재생/정지(한 번 멈추면 다시 켤 수 없던 오비트를 복구), 현위치(핀 + 이동, 실패 안내), 확대/축소(좁은 화면에서는 숨김, 손가락이 한다). 라벨은 세 언어, 남은 MapLibre 컨트롤(출처 토글)도 `locale` 로 번역. `maxPitch` 85→80: 컴패스 끌기로 85° 까지 올리면 熱海에서 카메라가 능선 안으로 들어가 화면이 줄무늬가 됐다.
