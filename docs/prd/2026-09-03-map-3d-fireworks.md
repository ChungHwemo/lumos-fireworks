...
> 2026-09-04 macOS Chrome 확인. 지형·밤하늘·오비트·픽토그램 핀·범례·360px 가로 스크롤 없음·`document.hidden` 정지 가드 통과.
> 불꽃은 진입 카메라가 지형에 파묻혀 처음엔 한 프레임도 안 보였고, 앵커 중심 구도로 고친 뒤 셸 궤적을 확인했다.
> 남은 것: 진입 줌에서 핀이 겹친다(라벨은 z-index 로 위에 남는다). 명당 번호끼리 겹치는 것은 이 사이클 이전부터 있던 문제다.
